...
// ============================================================
console.log("✅ Reports V2 loaded: reports-v2-safe-recovery-20260425");
...
(function () {
    ...
    function normalizeReportDate(d) {
        ...
        if (typeof d === "string") {
            ...
            // US-style mm/dd/yyyy from date input display or older records
    ...
    function getRowDateString(d) {
        return normalizeReportDate(
            d.dateString ||
            d.date ||
            d.bookingDate ||
            d.appointmentDate ||
    ...
    async function fetchDateRange(collection, start, end) {
        ...
        // Primary path: keep the original efficient dateString range query.
        try {
            ...
            snap.forEach(doc => {
                ...
                rows.push({
                    ...
                    _source: collection,
                    ...d,
                    dateString: getRowDateString(d) || d.dateString || ""
                });
            });
        } catch (e) {
            console.warn(`Reports V2 dateString query failed for ${collection}`, e);
        }

        // Fallback: only scan when the efficient query returned nothing.
        // This catches records saved with date / bookingDate instead of dateString.
        if (!rows.length) {
            try {
                const snap = await db.collection(collection).get();
                snap.forEach(doc => {
                    const d = doc.data() || {};
                    const rowDate = getRowDateString(d);
                ...
                console.warn(`Reports V2 fallback scan failed for ${collection}`, e);
                throw e;
            }
        }
    ...
    function filterRows(rows) {
        ...
        return rows.filter(r => {
            if (tech !== "all" && String(r.assignedTechEmail || "").trim().toLowerCase() !== String(tech).trim().toLowerCase()) return false;
    ...
    function renderShell() {
        ...
        setRangeUI(RPT.activeTab === "monthly" ? "month" : RPT.activeTab === "daily" ? "today" : "week");
